...
def replace_custom_links(md_text):
    ...
    def replacer(match):
        ...
        if kind == "category":
            ...
            query = splat_query(label, *(["-obsolete", "-incomplete", "package"] if label in main_types else []))
    ...
    new_md += "\n\n"
    for ref, url in link_defs.items():
        new_md += f"[{ref}]: {url}\n"
